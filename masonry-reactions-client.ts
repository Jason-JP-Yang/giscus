...
(function () {
  ...
  let reactionsData: {
    ...
    imageReactions: Record<string, { commentId: string; heartCount: number }>;
  };
  ...
  if (!reactionsData || !reactionsData.imageReactions) return;
  ...
  let viewerReactions: Record<string, boolean> = loadViewerReactions();
  ...
  function loadViewerReactions(): Record<string, boolean> {
    try {
  ...
  function saveViewerReactions(): void {
    try {
   ...
   */
  async function fetchLiveReactions(token: string): Promise<CommentNode[] | null> {
    if (!repoOwner || !repoName || !discussionNumber) return null;
   ...
   */
  function parseImageIdFromBody(body: string): string | null {
    if (!body) return null;
   ...
   */
  function createHeartButton(imageId: string): HTMLButtonElement | null {
   ...
   */
  function enableHeartButtons(): void {
    document.querySelectorAll<HTMLButtonElement>(".masonry-heart-btn").forEach((btn) => {
      btn.disabled = false;
  ...
  function showLoginPrompt(): void {
    if (document.querySelector(".masonry-login-prompt")) return;
   ...
   */
  function applyLocalViewerState(): void {
   ...
   */
  async function tryAuthenticate(retryCount = 0): Promise<void> {
    try {
