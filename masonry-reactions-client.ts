...
(function () {
  ...
  interface CachedData {
    timestamp: number;
    imageReactions: Record<string, ImageReaction>;
  }

  /* ==================== State ==================== */

  let currentPagePath = "";
  let imageReactions: Record<string, ImageReaction> = {};
   ...
   */
  function getGiscusApiBase(): string {
   ...
   */
  async function getGiscusToken(): Promise<string | null> {
   ...
   */
  function getGiscusLoginUrl(): string {
   ...
   */
  async function fetchFromGiscusAPI(
    repo: string,
    term: string,
    category: string,
    first: number = 100,
  ...
  async function fetchAllComments(
    ...
    if (!firstPage?.discussion) return null;

    const allComments: any[] = [...(firstPage.discussion.comments || [])];
    let pageInfo = firstPage.discussion.pageInfo;

    // Paginate if there are more than 100 comments
    while (pageInfo?.hasNextPage && pageInfo.endCursor) {
      const nextPage = await fetchFromGiscusAPI(
        repo,
        term,
        category,
        100,
   ...
   */
  async function checkViewerReactions(
    commentIds: string[],
    token: string
    ...
    for (let i = 0; i < commentIds.length; i += BATCH) {
      ...
      const aliases = batch
        .map(
          (id, j) =>
            `c${j}: node(id: "${id}") { ... on DiscussionComment { id reactionGroups { content viewerHasReacted } } }`
        )
        .join("\n");
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          ...
          if (node?.reactionGroups) {
            const heart = node.reactionGroups.find(
              (g: any) => g.content === "HEART"
            );
   ...
   */
  async function toggleHeartReaction(
    token: string,
    commentId: string,
    hasReacted: boolean
  ): Promise<boolean> {
   ...
   */
  function parseImageId(bodyHTML: string): string | null {
  ...
  function getCache(term: string): CachedData | null {
    try {
  ...
  function setCache(term: string, data: CachedData): void {
    try {
      sessionStorage.setItem(getCacheKey(term), JSON.stringify(data));
    } catch {}
  }

  function clearCache(term: string): void {
    try {
      sessionStorage.removeItem(getCacheKey(term));
    } catch {}
  }
   ...
   */
  function createHeartButton(imageId: string): HTMLButtonElement {
    ...
    btn.addEventListener("click", handleHeartClick as EventListener);
    return btn;
  }
   ...
   */
  function initializeHeartButtons(imageIds: string[]): void {
    ...
    items.forEach((container) => {
      ...
      if (container.querySelector(".masonry-heart-btn")) return;
  ...
  function updateHeartButton(
    ...
    const btn = document.querySelector<HTMLButtonElement>(
      `.masonry-heart-btn[data-image-id="${CSS.escape(imageId)}"]`
    );
    if (!btn) return;

    const countEl = btn.querySelector(".heart-count");
    if (countEl) countEl.textContent = String(heartCount);
   ...
   */
  function applyReactions(comments: any[]): void {
    imageReactions = {};
    ...
    for (const comment of comments) {
      ...
      const viewerHasReacted =
        comment.reactions?.HEART?.viewerHasReacted || false;
   ...
   */
  function applyCachedReactions(cached: CachedData): void {
   ...
   */
  async function handleHeartClick(e: Event): Promise<void> {
    e.preventDefault();
    e.stopPropagation();
    ...
    const success = await toggleHeartReaction(
      userToken,
      reaction.commentId,
      currentlyReacted
    );
    ...
    if (success) {
      // Invalidate cache
      const config = getPageConfig();
      if (config) clearCache(config.discussionTerm);
    } else {
      // Revert on failure
  ...
  async function init(): Promise<void> {
    ...
    if (isInitialized && currentPagePath === newPagePath) return;
    ...
    if (pendingHeart) sessionStorage.removeItem(PENDING_HEART_KEY);
    const savedScroll = sessionStorage.getItem(MASONRY_SCROLL_KEY);
    if (savedScroll) sessionStorage.removeItem(MASONRY_SCROLL_KEY);
   ...
   */
  function handlePendingHeart(imageId: string): void {
    setTimeout(() => {
      const btn = document.querySelector<HTMLButtonElement>(
        `.masonry-heart-btn[data-image-id="${CSS.escape(imageId)}"]`
      );
    ...
    try {
      ...
      if (isAuthenticated && userToken) {
        ...
        if (commentIds.length > 0) {
          const viewerReactions = await checkViewerReactions(
            commentIds,
            userToken
          );
          for (const [imageId, data] of Object.entries(imageReactions)) {
