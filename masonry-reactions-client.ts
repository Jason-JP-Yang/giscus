...
(function () {
  ...
  async function fetchDiscussionData(
    ...
    while (pageInfo?.hasNextPage) {
      const pageResult = await graphqlFetch(
      ...
      if (pageResult.errors) break;

      const moreComments =
        pageResult.data?.repository?.discussion?.comments;
   ...
   */
  function parseImageId(body: string): string | null {
    if (!body) return null;
   ...
   */
  function applyReactions(comments: CommentNode[]): void {
    ...
    for (const comment of comments) {
      ...
      const heartGroup = comment.reactionGroups?.find(
        (g) => g.content === "HEART"
      );
